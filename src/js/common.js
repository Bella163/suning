...
var checkReg = {
	...
	psw:function(str){
        var reg = /^\s+|\s+$/gi;
        var res = str.replace(reg,"");//去掉前后空格
        var str2 = res;//存储，用于密码确认
        var level = 0;//初始密码等级为0；
        if(res.length <= 20 && res.length >=6){//长度在6-20，且没有中文时执行；
            /\d/.test(res)?level++:null;//存在数字，等级加1；
            /[\W_]/.test(res)?level++:null;//存在特殊字符，等级加1；
            /[a-zA-Z]/.test(res)?level++:null;//存在字母，等级加1；
            if (/[\u2E80-\u9FFF]/.test(res)) {
                return {level:0,str2:str2};
            }
        } else { 
            return {level:0,str2:str2};
        }
        return {level:level,str2:str2};
    }
}
